...
rgb = np.array(im)
grey = np.array(0.2989 * rgb[:, :, 0] + 0.5870 *
                rgb[:, :, 1] + 0.1140 * rgb[:, :, 2]).astype(np.uint8)

grey = np.expand_dims(np.expand_dims(grey, axis=0), axis=-1)

tf_input_image = tf.constant(grey/255.0, dtype=tf.float32)
tf_reference_image = tf.constant(
    np.expand_dims(rgb/255.0, axis=0), dtype=tf.float32)

output = module.lattice_filter(
    tf_input_image, tf_reference_image, theta_alpha=theta_alpha, theta_beta=theta_beta)
