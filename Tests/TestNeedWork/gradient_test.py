...
class HighDimGradTest(tf.test.TestCase):
    ...
    def test_high_dim_filter_grad(self):
        ...
        rgb_np = np.random.randint(
            low=0, high=256, size=rgb_shape).astype(np.float32)  # / 256.0

        with self.test_session():
            unary_tf = constant_op.constant(unary_np)
            rgb_tf = constant_op.constant(rgb_np)
            y_tf = custom_module.high_dim_filter2d(
                unary_tf, rgb_tf, reverse=True)

            out = gradient_checker.compute_gradient(
                [unary_tf, rgb_tf], [x_shape, rgb_shape], y_tf, x_shape)
            ...
            print('Gradient check: measure1 = {:.6f}, measure2 = {:.6f}'.format(
                measure1, measure2))
            self.assertLess(
                measure1, 1e-3, 'Errors found in the gradient computation.')
            self.assertLess(
                measure2, 2e-2, 'Errors found in the gradient computation.')
